...
verify_hadoop_config() {
    local configs=(
        ...
        "hadoop getconf -confKey mapreduce.framework.name|yarn"
        "hadoop getconf -confKey mapreduce.jobhistory.address|hadoop-master:10020"
        "hadoop getconf -confKey mapreduce.task.io.sort.mb|256"
        "yarn getconf -confKey yarn.resourcemanager.address|hadoop-master:8032"
        "yarn getconf -confKey yarn.nodemanager.resource.memory-mb|8192"
        "yarn getconf -confKey yarn.scheduler.minimum-allocation-mb|1024"
    )
